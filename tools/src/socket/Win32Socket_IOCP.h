...
public:
	...
	void AddSocket(CWinSocket* newsock);
	void DelSocket(CWinSocket* sock);
...
private:
	...
	static void shutdown(CWinSocket* sock);
	static void OnWrite(CWinSocket* sock,int len);
	HANDLE* m_handleList;
	static HANDLE m_hIOCP;
	int m_nCount;
	std::set<CWinSocket*> m_WinSocketList;
	std::set<CWinSocket*> m_delList;
};
...
protected:
	bool pushReadEvent();
	Message_Block m_writeBlock;
	Message_Block m_readBlock;
