...
#include <queue>
#include <list>
#include "define_allocator.h"
template<class T,class MUTEX>
class Msg_Queue
{
public:
	Msg_Queue(uint32_t maxsize=1024*1024,MUTEX* _mutex=NULL):m_sama(0),isdestory(false)
	{
		if(_mutex)m_mutex_read=_mutex;
		else  m_mutex_read=new MUTEX;
		m_mutex_write=new Thread_Mutex();
		m_Queue=new std::list<T* >();
		m_sama=new Thread_Semaphore(0,maxsize);
	}
	~Msg_Queue()
	{
...
private:
	...
	std::list<T* >* m_Queue;
	//std::queue<T*/*,user_defined_allocator<T*>*/ > m_Queue;
	bool isdestory;
};
